...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EikthyrnirSettings {
    pub grpc_port: u16,
    pub database: DatabaseConfig,
    pub redis: RedisConfig,
}

impl Default for EikthyrnirSettings {
    fn default() -> Self {
        Self {
            grpc_port: 50064,
            database: DatabaseConfig {
                url: "postgres://localhost/eikthyrnir".to_string(),
                max_connections: 10,
                min_connections: 2,
            },
            redis: RedisConfig {
...
pub struct SettingsManager {
    settings: Arc<RwLock<EikthyrnirSettings>>,
    config_path: PathBuf,
}
...
impl SettingsManager {
    pub fn new(config_path: PathBuf) -> Self {
        Self {
            settings: Arc::new(RwLock::new(EikthyrnirSettings::default())),
            config_path,
        }
    }
    ...
    pub async fn load(&self) -> Result<(), SettingsError> {
        if !self.config_path.exists() {
            let default_settings = EikthyrnirSettings::default();
        ...
        let settings: EikthyrnirSettings = serde_json::from_str(&content)?;
    ...
    pub async fn get(&self) -> EikthyrnirSettings {
    ...
    pub fn validate(&self, settings: &EikthyrnirSettings) -> Result<(), SettingsError> {
                    ...
                    if matches!(event.kind, EventKind::Modify(_)) {
                        ...
                        if path == config_path {
                            ...
                            if let Err(e) = rt.block_on(async {
                                ...
                                let new_settings: EikthyrnirSettings = serde_json::from_str(&content)?;
