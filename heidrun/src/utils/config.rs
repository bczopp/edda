...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HeidrunSettings {
    pub grpc_port: u16,
    pub database: DatabaseConfig,
    pub commission_rate: f64,
}

impl Default for HeidrunSettings {
    fn default() -> Self {
        Self {
            grpc_port: 50063,
            database: DatabaseConfig {
                url: "postgres://localhost/heidrun".to_string(),
                max_connections: 10,
                min_connections: 2,
            },
            commission_rate: 0.15, // 15% as per plan
        }
    }
}
...
pub struct SettingsManager {
    settings: Arc<RwLock<HeidrunSettings>>,
    config_path: PathBuf,
}
...
impl SettingsManager {
    pub fn new(config_path: PathBuf) -> Self {
        Self {
            settings: Arc::new(RwLock::new(HeidrunSettings::default())),
            config_path,
        }
    }
    ...
    pub async fn load(&self) -> Result<(), SettingsError> {
        if !self.config_path.exists() {
            let default_settings = HeidrunSettings::default();
        ...
        let settings: HeidrunSettings = serde_json::from_str(&content)?;
    ...
    pub async fn get(&self) -> HeidrunSettings {
    ...
    pub fn validate(&self, settings: &HeidrunSettings) -> Result<(), SettingsError> {
                    ...
                    if matches!(event.kind, EventKind::Modify(_)) {
                        ...
                        if path == config_path {
                            ...
                            if let Err(e) = rt.block_on(async {
                                ...
                                let new_settings: HeidrunSettings = serde_json::from_str(&content)?;
