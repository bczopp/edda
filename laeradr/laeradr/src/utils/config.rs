...
impl Default for LaeradrSettings {
    fn default() -> Self {
        Self {
            grpc_port: 50065,
            dainn: DainnConfig {
            ...
            durathror: DurathrorConfig {
                ...
                s3_bucket: "laeradr-archive".to_string(),
            },
        }
    }
}
...
pub struct SettingsManager {
    settings: Arc<RwLock<LaeradrSettings>>,
    config_path: PathBuf,
}
...
impl SettingsManager {
    pub fn new(config_path: PathBuf) -> Self {
        Self {
            settings: Arc::new(RwLock::new(LaeradrSettings::default())),
            config_path,
        }
    }
    ...
    pub async fn load(&self) -> Result<(), SettingsError> {
        if !self.config_path.exists() {
            let default_settings = LaeradrSettings::default();
        ...
        let settings: LaeradrSettings = serde_json::from_str(&content)?;
    ...
    pub async fn get(&self) -> LaeradrSettings {
    ...
    pub fn validate(&self, settings: &LaeradrSettings) -> Result<(), SettingsError> {
                    ...
                    if matches!(event.kind, EventKind::Modify(_)) {
                        ...
                        if path == config_path {
                            ...
                            if let Err(e) = rt.block_on(async {
                                ...
                                let new_settings: LaeradrSettings = serde_json::from_str(&content)?;
