...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AsgardSettings {
    pub odin: OdinConfig,
    pub server: ServerConfig,
}

impl Default for AsgardSettings {
    fn default() -> Self {
        Self {
            odin: OdinConfig {
...
pub struct SettingsManager {
    settings: Arc<RwLock<AsgardSettings>>,
    config_path: PathBuf,
}
...
impl SettingsManager {
    pub fn new(config_path: PathBuf) -> Self {
        Self {
            settings: Arc::new(RwLock::new(AsgardSettings::default())),
            config_path,
        }
    }
    ...
    pub async fn load(&self) -> Result<(), SettingsError> {
        if !self.config_path.exists() {
            let default_settings = AsgardSettings::default();
        ...
        let settings: AsgardSettings = serde_json::from_str(&content)?;
    ...
    pub async fn get(&self) -> AsgardSettings {
    ...
    pub fn validate(&self, settings: &AsgardSettings) -> Result<(), SettingsError> {
        if settings.odin.port == 0 {
            return Err(SettingsError::ValidationError("odin.port cannot be 0".to_string()));
        }
        if settings.server.port == 0 {
            return Err(SettingsError::ValidationError("server.port cannot be 0".to_string()));
        }
        Ok(())
    }
                    ...
                    if matches!(event.kind, EventKind::Modify(_)) {
                        ...
                        if path == config_path {
                            ...
                            if let Err(e) = rt.block_on(async {
                                ...
                                let new_settings: AsgardSettings = serde_json::from_str(&content)?;
