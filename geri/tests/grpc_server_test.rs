...
use geri::llm::{LocalLLMProvider, LLMProvider};
...
async fn test_process_prompt_returns_response_with_text_and_model() {
    ...
    let svc = GeriServiceImpl::new(llm.clone(), vision);
    ...
    let res = svc.process_prompt(req).await.expect("process_prompt");
...
async fn test_process_vision_returns_description_and_model() {
    ...
    let vision = Arc::new(VisionProcessor::new("test-vision".to_string()));
    let svc = GeriServiceImpl::new(llm, vision);
    ...
    let res = svc.process_vision(req).await.expect("process_vision");
    let out = res.into_inner();
