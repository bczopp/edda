...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ForsetiSettings {
    pub grpc_port: u16,
    pub database: DatabaseConfig,
    pub python: PythonConfig,
}

impl Default for ForsetiSettings {
    fn default() -> Self {
        Self {
            grpc_port: 50068,
            database: DatabaseConfig {
                url: "postgres://localhost/forseti".to_string(),
                max_connections: 10,
                min_connections: 2,
            },
            python: PythonConfig {
...
pub struct SettingsManager {
    settings: Arc<RwLock<ForsetiSettings>>,
    config_path: PathBuf,
}
...
impl SettingsManager {
    pub fn new(config_path: PathBuf) -> Self {
        Self {
            settings: Arc::new(RwLock::new(ForsetiSettings::default())),
            config_path,
        }
    }
    ...
    pub async fn load(&self) -> Result<(), SettingsError> {
        if !self.config_path.exists() {
            let default_settings = ForsetiSettings::default();
        ...
        let settings: ForsetiSettings = serde_json::from_str(&content)?;
    ...
    pub async fn get(&self) -> ForsetiSettings {
    ...
    pub fn validate(&self, settings: &ForsetiSettings) -> Result<(), SettingsError> {
                    ...
                    if matches!(event.kind, EventKind::Modify(_)) {
                        ...
                        if path == config_path {
                            ...
                            if let Err(e) = rt.block_on(async {
                                ...
                                let new_settings: ForsetiSettings = serde_json::from_str(&content)?;
